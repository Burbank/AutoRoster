...
# AutoRoster - AI-Powered Calendar Event Creator
...
readonly LOG_DIR="$HOME/Library/Logs/AutoRoster"
readonly LOG_FILE="$LOG_DIR/autoroster.log"
...
initialize() {
    log "INFO" "Starting AutoRoster (LLMCal fork)"
    ...
    if ! validate_dependencies; then
        show_error_notification "AutoRoster Setup"
        graceful_exit
    fi
    ...
    if ! test_calendar_availability; then
        show_error_notification "AutoRoster Calendar Access"
...
show_processing_notification() {
    ...
    osascript -e "display notification \"$processing_msg\" with title \"AutoRoster\"" 2>/dev/null || true
}
...
show_success_notification() {
    ...
    osascript -e "display notification \"$msg\" with title \"AutoRoster\"" 2>/dev/null || true
}

# Show error notification
show_error_notification_with_message() {
    show_error_notification "${LAST_ERROR_MESSAGE:-$(get_translation "error")}" "AutoRoster"
}
...
main() {
    ...
    # Cleanup and exit
    cleanup
    log "INFO" "AutoRoster processing finished"
}
