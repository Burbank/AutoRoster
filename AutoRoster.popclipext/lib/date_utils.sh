...
# Convert datetime from one timezone to another (for flight bookings)
# Timezones come from the AI (start_timezone, end_timezone) - no local airport database needed
convert_datetime_between_timezones() {
    local input_datetime="$1"
    local from_timezone="$2"
    local to_timezone="${3:-$(get_system_timezone)}"
    
    date_log "INFO" "Converting $input_datetime from $from_timezone to $to_timezone"
    
    [ -z "$input_datetime" ] && return 1
    [ -z "$from_timezone" ] && from_timezone="$(get_system_timezone)"
    
    local input_cleaned
    # Only add :00 when time is HH:MM (not HH:MM:SS) - avoid matching "30:00" in "12:30:00"
    input_cleaned=$(echo "$input_datetime" | tr -d '"')
    [[ "$input_cleaned" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}\ [0-9]{2}:[0-9]{2}$ ]] && input_cleaned="${input_cleaned}:00"
    
    local timestamp
    if timestamp=$(TZ="$from_timezone" date -j -f "%Y-%m-%d %H:%M:%S" "$input_cleaned" "+%s" 2>/dev/null); then
        :
    elif timestamp=$(TZ="$from_timezone" date -j -f "%Y-%m-%d %H:%M" "$input_cleaned" "+%s" 2>/dev/null); then
        :
    else
        date_log "WARN" "Could not parse $input_cleaned in $from_timezone, using system TZ"
        timestamp=$(date -j -f "%Y-%m-%d %H:%M:%S" "$input_cleaned" "+%s" 2>/dev/null) || \
        timestamp=$(date -j -f "%Y-%m-%d %H:%M" "$input_cleaned" "+%s" 2>/dev/null)
    fi
    
    [ -z "$timestamp" ] && return 1
    
    local converted
    converted=$(TZ="$to_timezone" date -r "$timestamp" "+$DATE_FORMAT_OUTPUT" 2>/dev/null)
    [ -z "$converted" ] && return 1
    
    date_log "INFO" "Converted: $input_datetime ($from_timezone) -> $converted ($to_timezone)"
    echo "$converted"
    return 0
}
...
date_log() {
    local level="$1"
    local message="$2"
    echo "$(date '+%Y-%m-%d %H:%M:%S') [DATE-$level]: $message" >> "${LOG_FILE:-/tmp/autoroster.log}"
}
...
convert_datetime() {
    ...
    # Clean input datetime - only add :00 when time is HH:MM (not HH:MM:SS)
    local cleaned_datetime
    cleaned_datetime=$(echo "$input_datetime" | tr -d '"')
    [[ "$cleaned_datetime" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}\ [0-9]{2}:[0-9]{2}$ ]] && cleaned_datetime="${cleaned_datetime}:00"
...
export -f convert_datetime_between_timezones