...
show_error_notification() {
    ...
    local title="${2:-AutoRoster Error}"
...
show_recovery_suggestion() {
    ...
    if [ -n "$suggestion" ]; then
        osascript -e "display dialog \"$suggestion\" buttons {\"OK\"} default button 1 with title \"AutoRoster - Error Details\"" 2>/dev/null || true
    fi
}

# Create error report for debugging
create_error_report() {
    local report_file="${1:-$HOME/Library/Logs/AutoRoster/error_report_$(date +%Y%m%d_%H%M%S).txt}"
    local report_dir
    report_dir=$(dirname "$report_file")
    mkdir -p "$report_dir"
    
    {
        echo "AutoRoster Error Report"
        echo "=================="
        echo "Date: $(date)"
...
# Anthropic keys can have varying formats: sk-ant-, sk-ant-api03-, etc.
# Accept any key that starts with sk-ant- and has sufficient length
validate_api_key() {
    local api_key="$1"
    
    # Trim whitespace that may have been accidentally copied
    ...
    # Check for Anthropic API key prefix (sk-ant-); accept various key formats
    if [[ ! "$api_key" =~ ^sk-ant- ]]; then
        handle_error "$ERR_API_KEY_MISSING" "Invalid API key format (should start with sk-ant-)"
        return "$ERR_API_KEY_MISSING"
    fi
    
    # Ensure minimum length for a valid key
    if [ ${#api_key} -lt 20 ]; then
        handle_error "$ERR_API_KEY_MISSING" "API key appears truncated"
        return "$ERR_API_KEY_MISSING"
    fi
