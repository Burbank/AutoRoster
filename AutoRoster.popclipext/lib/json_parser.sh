...
readonly JSON_TEMP_DIR="/tmp/autoroster_json"
readonly PYTHON_JSON_PROCESSOR="/tmp/autoroster_json_processor.py"
...
JSON_CACHE_DIR="/tmp/autoroster_json_cache"
...
json_log() {
    local level="$1"
    local message="$2"
    echo "$(date '+%Y-%m-%d %H:%M:%S') [JSON-$level]: $message" >> "${LOG_FILE:-/tmp/autoroster.log}"
}
...
def extract_text_from_content_blocks(content_list: list) -> str:
    """Extract text from Anthropic API content blocks. Handles both block formats."""
    text_parts = []
...
def process_anthropic_response(response_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(response_data, dict):
            # Check if it's already an event object (direct JSON response)
            ...
            if 'content' in response_data:
                ...
                if content_list and isinstance(content_list, list):
                    text_content = extract_text_from_content_blocks(content_list)
                    if text_content:
...
def validate_event_data(event_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Check for start_time (required)
    if 'start_time' not in event_data or not str(event_data.get('start_time', '')).strip():
        print("Missing required field: start_time", file=sys.stderr)
        return {}
    
    # Normalize and validate fields
    normalized_event = {}
    
    # Title - required but provide fallback when empty (fixes "Unable to create a title" error)
    title = str(event_data.get('title', '')).strip()
    if not title:
        # Try description as fallback, or use generic title
    ...
    # End time - required for calendar; default to 1 hour after start if missing
    end_time = str(event_data.get('end_time', '')).strip()
    if not end_time:
        try:
            from datetime import datetime, timedelta
            st_str = normalized_event['start_time'].split('.')[0]
            for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%d"):
                try:
                    st = datetime.strptime(st_str, fmt)
                    end_time = (st + timedelta(hours=1)).strftime("%Y-%m-%d %H:%M")
                    break
                except ValueError:
                    pass
            if not end_time:
                end_time = normalized_event['start_time']
        except ImportError:
            end_time = normalized_event['start_time']
    ...
    normalized_event['start_timezone'] = str(event_data.get('start_timezone', '')).strip()
    normalized_event['end_timezone'] = str(event_data.get('end_timezone', '')).strip()
