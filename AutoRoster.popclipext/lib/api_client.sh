...
API_CACHE_DIR="/tmp/autoroster_api_cache"
...
api_log() {
    local level="$1"
    local message="$2"
    echo "$(date '+%Y-%m-%d %H:%M:%S') [API-$level]: $message" >> "${LOG_FILE:-/tmp/autoroster.log}"
}
...
validate_anthropic_api_key() {
    ...
    api_log "INFO" "Validating Anthropic API key (length: ${#api_key} chars)"
    
    if ! validate_api_key "$api_key"; then
        return $(get_error_code)
    fi
    
    # Skip the test API call - it can fail due to network/timeout and blocks usage.
    # The actual request will validate the key; invalid keys get 401 response.
    api_log "INFO" "API key format valid, proceeding"
    return $ERR_SUCCESS
}
...
Return ONLY valid JSON in this exact format:
{
    ...
    "excluded_dates": [],
    "attendees": [],
    "start_timezone": "IANA timezone if flight, e.g. Europe/Amsterdam",
    "end_timezone": "IANA timezone if flight, e.g. Asia/Hong_Kong"
}

FLIGHT/TIMEZONE RULES - CRITICAL when airport codes (AMS, LAX, HKG, etc.) appear:
- start_time is ALWAYS in the DEPARTURE airport's local timezone (e.g. AMS 12:30 = 12:30 Europe/Amsterdam)
- end_time is ALWAYS in the ARRIVAL airport's local timezone (e.g. HKG 07:30 +1 = next day 07:30 Asia/Hong_Kong)
- Set start_timezone to the departure airport's IANA timezone (e.g. "Europe/Amsterdam" for AMS)
- Set end_timezone to the arrival airport's IANA timezone (e.g. "Asia/Hong_Kong" for HKG)
- Use your knowledge of all IATA codes and their timezones - include the correct IANA zone
- For non-flights, leave start_timezone and end_timezone empty or omit them
