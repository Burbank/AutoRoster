...
calendar_log() {
    local level="$1"
    local message="$2"
    echo "$(date '+%Y-%m-%d %H:%M:%S') [CALENDAR-$level]: $message" >> "${LOG_FILE:-/tmp/autoroster.log}"
}
...
create_calendar_event() {
    ...
    # Resolve calendar name to one that exists (user may not have "Personal", "Work", etc.)
    if [ -n "$calendar_name" ]; then
        calendar_name=$(resolve_calendar_name "$calendar_name")
    fi
    if [ -z "$calendar_name" ]; then
        calendar_name=$(resolve_calendar_name "Day2Day")
    fi
    if [ -z "$calendar_name" ]; then
        handle_error $ERR_CALENDAR_CREATION_FAILED "No calendar available"
        return $ERR_CALENDAR_CREATION_FAILED
    fi
...
# Resolve requested calendar name to one that actually exists
# Many users don't have "Personal", "Work", etc. - use first available as fallback
resolve_calendar_name() {
    local requested="$1"
    
    local calendars_list
    calendars_list=$(osascript -e 'tell application "Calendar" to get name of every calendar' 2>/dev/null)
    if [ -z "$calendars_list" ]; then
        echo ""
        return 1
    fi
    
    # AppleScript returns comma-separated list; normalize to newlines
    local calendars_one_per_line
    calendars_one_per_line=$(echo "$calendars_list" | tr ',' '\n' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | grep -v '^$')
    
    # Check if requested name exists (exact match)
    local matched
    matched=$(echo "$calendars_one_per_line" | grep -Fxs "$requested" | head -1)
    if [ -n "$matched" ]; then
        echo "$matched"
        return 0
    fi
    ...
    matched=$(echo "$calendars_one_per_line" | while IFS= read -r cal; do
        [ "$(echo "$cal" | tr '[:upper:]' '[:lower:]')" = "$(echo "$requested" | tr '[:upper:]' '[:lower:]')" ] && echo "$cal" && break
    done)
    if [ -n "$matched" ]; then
        echo "$matched"
        return 0
    ...
    # Map requested names to likely alternatives for this user's setup
    local alternatives=""
    case "$(echo "$requested" | tr '[:upper:]' '[:lower:]')" in
        "personal")
            alternatives="Family Day2Day Household Amelie Arie main gcal"
            ;;
        "work")
            alternatives="work Work crewapp Day2Day"
            ;;
        "high priority"|"medium priority"|"low priority"|"deadlines"|"meetings")
            alternatives="Day2Day Family work Arie main gcal"
            ;;
        *)
            alternatives="Day2Day Arie main gcal Family work"
            ;;
    esac
    
    # Try each alternative (partial match - calendar name contains the string)
    for alt in $alternatives; do
        matched=$(echo "$calendars_one_per_line" | while IFS= read -r cal; do
            echo "$cal" | grep -qiF "$alt" && echo "$cal" && break
        done)
        if [ -n "$matched" ]; then
            calendar_log "INFO" "Calendar \"$requested\" not found, using \"$matched\""
            echo "$matched"
            return 0
        fi
    done
    ...
    local first_calendar
    first_calendar=$(echo "$calendars_one_per_line" | head -1)
    if [ -n "$first_calendar" ]; then
        calendar_log "INFO" "Calendar \"$requested\" not found, using first: \"$first_calendar\""
        echo "$first_calendar"
        return 0
    fi
