...
show_recovery_suggestion() {
    ...
    if [ -n "$suggestion" ]; then
        osascript -e "display dialog \"$suggestion\" buttons {\"OK\"} default button 1 with title \"AutoCal - Error Details\"" 2>/dev/null || true
    fi
}

# Create error report for debugging
create_error_report() {
    local report_file="${1:-$HOME/Library/Logs/AutoCal/error_report_$(date +%Y%m%d_%H%M%S).txt}"
    local report_dir
    report_dir=$(dirname "$report_file")
    mkdir -p "$report_dir"
    
    {
        echo "AutoCal Error Report"
        echo "=================="
        echo "Date: $(date)"
